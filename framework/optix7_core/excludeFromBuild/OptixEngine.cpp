...
ProgramGroupHandle OptixEngine::createExceptionPrograms(ModuleHandle& module, const String& functionName, OptixConfig& config)
{
	config.desc.exception_prog_group_desc.kind = OPTIX_PROGRAM_GROUP_KIND_EXCEPTION;
	config.desc.exception_prog_group_desc.exception.module = module ? module->get() : nullptr;
	config.desc.exception_prog_group_desc.exception.entryFunctionName = functionName.length() ? functionName.getCharPointer().getAddress() : nullptr;

	ProgramGroupHandle handle = OptiXProgramGroup::create(config.options.program_group_options, config.desc.exception_prog_group_desc);
	handle->makeGroup(context);
...
void OptixEngine::createProgramGroups(const json& groups,  OptixConfig& config)
{
	...
	for (auto it : programDB)
	{
		...
		if (!module)
			throw std::runtime_error("Failed to create module: " + moduleName.toStdString());
...
void OptixEngine::createProgramGroup(const json& j, OptixConfig& config)
{
	...
	switch (kind)
	{
		...
		case OPTIX_PROGRAM_GROUP_KIND_EXCEPTION:
		{
			...
			{
				ModuleHandle null = nullptr;
				ProgramGroupHandle handle = createExceptionPrograms(null, "", config);
				if (handle)
					config.programs.insert(std::make_pair(name, handle));
				break;
			}
			...
			if (it != modules.end())
			{
				...
				ProgramGroupHandle handle = createExceptionPrograms(it->second, funcName, config);
				if (handle)
					config.programs.insert(std::make_pair(name, handle));
			}
			break;
