...
{
    static void createSmallSieve (const int numBits, juce::BigInteger& result)
    {
    ...
    static void bigSieve (const juce::BigInteger& base, const int numBits, juce::BigInteger& result,
                          const juce::BigInteger& smallSieve, const int smallSieveSize)
    {
        ...
        do
        {
            ...
            juce::BigInteger r (base), remainder;
    ...
    static bool findCandidate (const juce::BigInteger& base, const juce::BigInteger& sieve,
                               const int numBits, juce::BigInteger& result, const int certainty)
    {
        for (int i = 0; i < numBits; ++i)
        {
            if (! sieve[i])
            {
                ...
                if (Prime::isProbablyPrime (result, certainty))
                    return true;
            }
        }
    ...
    static bool passesMillerRabin (const juce::BigInteger& n, int iterations)
    {
        const juce::BigInteger one (1), two (2);
        const juce::BigInteger nMinusOne (n - one);

        juce::BigInteger d (nMinusOne);
        const int s = d.findNextSetBit (0);
        d >>= s;

        juce::BigInteger smallPrimes;
...
//==============================================================================
juce::BigInteger Prime::createProbablePrime (const int bitLength,
                                        const int certainty,
...
bool Prime::isProbablyPrime (const juce::BigInteger& number, const int certainty)
{
    using namespace PrimesHelpers;
